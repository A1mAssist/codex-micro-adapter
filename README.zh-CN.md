...
绑定语法刻意做得很小：`mod+mod+key`、`type:<字面文本>`、`url:<https 地址>`，以及
`hold:<组合键>` —— 按住时按下、松手时抬起，按住期间像真键盘一样自动重复。任何槽位都能
用任意一种，麦克风键并不特殊：它只是一个碰巧默认绑了按住动作的普通键。把 `MIC` 键帽
换到别的槽位、或者给别的键绑 `hold:...`，行为完全一样。

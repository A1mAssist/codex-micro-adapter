...
绑定语法刻意做得很小：`mod+mod+key`、`type:<字面文本>`、`url:<https 地址>`、
`hold:<组合键>`、`plugin:<事件>`。

- `hold:` —— 按住时按下、松手时抬起，按住期间像真键盘一样自动重复。任何槽位都能用，
  麦克风键并不特殊：它只是一个碰巧默认绑了按住动作的普通键。
- `plugin:` —— 不合成按键，而是把事件发布到控制端口的事件流上，由该 harness 自己的
  插件去调它的 API。Web UI 的按钮没有键位可绑的 harness（`dsh` 的审批）走这条路。
  页面只应答**用户正在看**的那个会话的审批，绝不碰后台会话。

没绑定的动作会被报出来，绝不会被默默吞掉。默认值几乎为空是有意的 —— 替别人的工具
发明键位就是瞎猜。
